...
void I2C_EV_IRQHandling(I2C_Handle_t *pI2CHandle)
{
    ...
    if (temp1 && temp2)
    {
        ...
        if (pI2CHandle->RxTxStatus == I2C_STATUS_BUSY_IN_TX)
        {
            ...
            I2C_ExecuteAddresPhase(pI2CHandle->pI2Cx, pI2CHandle->Target_Address, 0);
        }
        else if (pI2CHandle->RxTxStatus == I2C_STATUS_BUSY_IN_RX)
        {

            I2C_ExecuteAddresPhase(pI2CHandle->pI2Cx, pI2CHandle->Target_Address, 1);
        }
    }
        ...
        if ((pI2CHandle->pI2Cx->SR2 & (1 << I2C_SR2_MSL)))
        {
            ...
            if (pI2CHandle->RxTxStatus == I2C_STATUS_BUSY_IN_RX)
            {
                ...
                if (pI2CHandle->RxLen == 0)
                {
                    ...
                    // generate the stop condition
                    if (pI2CHandle->RepeatedStart == I2C_DISABLE_SR)
                    {
                        I2C_Generate_Stop_Condition(pI2CHandle->pI2Cx);
                    }
...
__attribute__((weak))void I2C_ApplicationEventCallback(I2C_Handle_t *pI2CHandle, uint8_t eventcode)
{
    if (eventcode == I2C_EV_RX_CMPLT)
        printf("Reception has completed \n ");
    else if (eventcode == I2C_EV_TX_CMPLT)
        printf("transmission has completed \n ");
    else if (eventcode == I2C_EV_STOPF)
